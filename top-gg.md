# 🛡️ Ryvex — Moderation You Can Audit, Trust, and Control

**Ryvex** is a **moderation-first Discord bot** built for servers that take  
**staff accountability, transparency, and long-term stability** seriously.

Ryvex is not another bloated “all-in-one” bot.  
It is a **structured moderation platform** designed to make every action  
**clear, traceable, and fair** — for both members *and* moderators.

If moderation matters in your server, Ryvex was built for you.

---

## 🎯 Who Ryvex Is For

Ryvex is designed for:

- Medium to large Discord servers
- Communities with **multiple moderators**
- Server owners who want **visibility and control**
- Teams that value **consistency over chaos**

If you want a casual, set-and-forget bot, Ryvex is probably not for you —  
and that’s intentional.

---

## 🧾 Case-Based Moderation — Transparency First

Every moderation action creates a **persistent case**:

- ⚠️ Warnings
- ⏳ Timeouts
- 👢 Kicks
- 🔨 Bans & unbans
- 🤖 AutoMod punishments
- 🔒 Channel & role enforcement

Each case records:
- Who acted
- Who was affected
- Why it happened
- When it happened
- Duration (when applicable)

Nothing is hidden. Nothing is overwritten.  
Perfect for **staff accountability, appeals, and audits**.

---

## 🤖 Smart AutoMod — Predictable, Not Random

Ryvex includes a **fully configurable AutoMod system** that protects your server from:

- 🚫 Spam
- 🔗 Unauthorized links
- 🤬 Offensive language (built-in + custom lists)

### What makes Ryvex AutoMod different?

- Unified warning counter across **all AutoMod triggers**
- **Escalating punishments that don’t reset**
- Quick presets: **Soft / Medium / Strict**
- Channel & role bypass support
- Optional **warn-only mode**

Members always know **where they stand**.  
Moderators always stay **in control**.

---

## 📊 Advanced Logging — Privacy Aware by Design

Ryvex uses a **dual logging system**:

- 📜 General server logs
- 🛡️ Moderation-only logs

Key features:
- Privacy-aware message logging
- Safe audit-log usage
- No duplicate or spammy logs
- Clear separation between events and moderation actions

Logging works **with Discord’s permission model**, never against it.

---

## 🧑‍⚖️ Staff Accountability & Monitoring

Ryvex includes **built-in staff oversight tools**:

- 📊 Staff activity dashboards
- 🧾 Moderator action summaries
- 🚨 Optional detection of unusual moderation patterns
- Full historical audit trail

Ryvex helps teams **self-regulate**, not police blindly.

---

## �️ Anti-Raid Protection

Protect your server from mass join attacks:

- Configurable join-rate thresholds
- Automatic detection and response
- Integrates with logging system

---

## �💤 AFK System — Stay Informed

- Set yourself AFK with `/afk [reason]`
- Automatically removed when you send a message
- Anyone who mentions you gets a notification with your AFK reason

---

## 🎭 Reaction Roles — Button-Based Self-Assignment

Create clean, button-based role panels — no emoji reactions needed.

- Custom embed title and description
- Add or remove roles per panel
- Members click a button to toggle roles

---

## 📊 Leveling / XP System — Engage Your Community

- Members earn XP per message (configurable range and cooldown)
- Level-up announcements with optional dedicated channel
- Automatic role rewards on level-up
- Server leaderboard with paginated display
- Full admin control: set, reset, ignore channels/roles

---

## 🎨 Rich Interactive Dashboards

Every informational command uses **multi-page interactive embeds** with ◀ ▶ navigation:

- `/botinfo` — 3 pages: Overview, Live Stats, Links & Info
- `/userinfo` — 3 pages: Identity, Moderation Snapshot, Leveling
- `/staff` — 3 pages: Overview, Leaderboard, Recent Activity
- `/changelog` — Timeline overview + per-release pages with stats
- `/setup` — 17-page interactive guide
- `/settings` — 10-page interactive dashboard
- `/rules` — Paginated display (3 per page)
- `/history` commands — Enriched entries with reason, timestamp, and duration

No clutter. No walls of text. Clean, navigable information.

---

## ⏱️ Slowmode

- `/slowmode <seconds>` — set channel slowmode as a logged moderation action

---

## 🛡️ Built for Stability

- All systems wrapped in crash protection with null-safe database calls
- Global error handlers prevent unexpected downtime
- Automatic memory management for long uptime
- AutoMod runs before XP — deleted messages never earn rewards
- Anti-raid kick targets all joiners, lock auto-restores after cooldown
- Slow commands use deferred replies — no silent timeouts
- No crashes, no silent failures

---

## ⚙️ Guided Setup — No Guesswork

Ryvex includes a **guided `/setup` dashboard** that shows:

- What is configured
- What is missing
- What is recommended
- Exactly which commands to run next

Nothing is changed automatically.  
Setup is **safe, visual, and repeatable**.

---

## 🧠 Built for Stability & Scale

- Modern slash commands only
- MongoDB-backed persistence
- Graceful permission fallbacks
- No crashes, no silent failures
- Designed to scale with growing communities

---

## 🔑 Key Commands (Quick Start)

- `/setup` — guided configuration
- `/settings` — server configuration overview
- `/automod` — AutoMod control & status
- `/case` — moderation case management
- `/modlog` — moderation history
- `/staff` — staff accountability dashboard
- `/slowmode` — set channel slowmode
- `/antiraid` — anti-raid protection config
- `/afk` — set yourself AFK with notifications
- `/reactionrole` — create button-based role panels
- `/rank` — view your level and XP
- `/leaderboard` — server XP leaderboard
- `/xp` — admin XP/leveling configuration
- `/review` — leave a review on Top.gg

---

## 🧩 Why Choose Ryvex?

✅ Rich interactive dashboards with page navigation  
✅ Anti-raid protection  
✅ Transparent moderation  
✅ Predictable AutoMod behavior  
✅ Full audit trail  
✅ Privacy-aware logging  
✅ Built for real moderation teams  
✅ AFK system with smart notifications  
✅ Button-based Reaction Roles  
✅ Leveling/XP system with role rewards  
✅ Crash-proof — never goes down unexpectedly  
✅ 54 slash commands and growing  

Ryvex doesn’t just moderate — it **documents, explains, and protects**.

---

## 🚀 Ready to Take Control?

Add Ryvex to your server and build moderation you can  
**trust, audit, and scale**.

If you want clarity instead of chaos —  
**Ryvex is built for you.**