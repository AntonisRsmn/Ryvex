...
module.exports = [
...

  {
    ...
    changes: [
      ...
      "🐛 Fixed `getGuildSettings` null crash — all callers now handle DB failures gracefully",
      "🐛 Fixed missing `await` in `updateGuildSettings` and `getUserModLogs` — error logging now works",
      "🐛 Fixed `getGuildSettings` writing to DB on every read — removed unnecessary save-on-read",
      "🐛 Fixed leaderboard and rank sorting by XP only — now sorts by level first, then XP",
      "🐛 Fixed XP awarded before AutoMod — automod now runs first, deleted messages skip XP",
      "🐛 Fixed anti-raid kick only targeting the triggering member — now kicks all recent joiners",
      "🐛 Fixed anti-raid lock never auto-restoring — verification level restores after 5 minutes",
      "🐛 Fixed missing optional chaining on `settings.welcome.enabled`",
      "⚡ Added `deferReply` to `/antiraid`, `/rank`, `/leaderboard` to prevent interaction timeouts",
      "📊 Updated `UserLevel` DB index to support level+XP compound sorting",
    ],
  },
