...
var DefaultDeviceProperty = `@discordjs/ws 2.0.4`;
...
var WebSocketShard = class extends import_async_event_emitter.AsyncEventEmitter {
  ...
  async destroy(options = {}) {
    ...
    if (this.connection) {
      ...
      if (shouldClose) {
        ...
        await promise;
        this.emit("closed" /* Closed */, options.code);
      }
      ...
      case import_v102.GatewayOpcodes.Dispatch: {
        ...
        switch (payload.t) {
          case import_v102.GatewayDispatchEvents.Ready: {
            ...
            this.emit("ready" /* Ready */, payload.d);
            break;
          }
        ...
        this.emit("dispatch" /* Dispatch */, payload);
        break;
      }
  ...
  onError(error) {
    this.emit("socketError" /* SocketError */, error);
  ...
  async onClose(code) {
    ...
    switch (code) {
      ...
      case import_v102.GatewayCloseCodes.InvalidShard: {
        this.emit("error" /* Error */, new Error("Invalid shard"));
        return this.destroy({ code });
      }
  ...
  debug(messages) {
    this.emit("debug" /* Debug */, messages.join("\n	"));
  }
};
